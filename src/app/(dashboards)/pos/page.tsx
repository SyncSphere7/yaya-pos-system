...
import { DepartmentSelector } from '@/components/pos/department-selector'
...
function POSInterface() {
  ...
  useEffect(() => {
    if (user?.locationId) {
      loadDepartments(user.locationId)
    }
  }, [user?.locationId, loadDepartments])

  useEffect(() => {
    if (user?.locationId && selectedDepartment) {
      loadProducts(user.locationId, selectedDepartment)
      loadCategories(user.locationId, selectedDepartment)
    }
  }, [user?.locationId, selectedDepartment, loadProducts, loadCategories])
      ...
      >
        <Toolbar sx={{ justifyContent: 'space-between', py: 2, px: 4 }}>
          <Box display="flex" alignItems="center" gap={3}>
            ...
            <Box>
              <Typography variant="h4" fontWeight={700} sx={{ color: 'text.primary', mb: 0.5 }}>
                {user?.organizationName || 'Yaya Xtra Residence POS'}
              </Typography>
              <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.875rem' }}>
                {selectedDepartmentName ? `${selectedDepartmentName} Terminal` : 'Point of Sale System'}
              </Typography>
            </Box>
          </Box>
          ...
          <Box display="flex" alignItems="center" gap={4}>
            ...
            >
              {[
                { type: 'dine_in', label: 'Dine In', icon: <DiningIcon /> },
                { type: 'takeout', label: 'Takeout', icon: <TakeoutIcon /> },
                { type: 'delivery', label: 'Delivery', icon: <DeliveryIcon /> }
              ].map(({ type, label, icon }) => (
                <Button
                  key={type}
                  startIcon={icon}
                  ...
                  sx={{
                    ...
                    fontWeight: 600,
                    textTransform: 'none',
                    bgcolor: orderType === type ? '#1a1a1a' : 'transparent',
                    color: orderType === type ? 'white' : 'text.primary',
                    boxShadow: orderType === type ? '0 4px 12px rgba(0, 0, 0, 0.15)' : 'none',
                    '&:hover': {
                      bgcolor: orderType === type ? '#2d2d2d' : 'rgba(0, 0, 0, 0.04)',
            ...
            {orderType === 'dine_in' && (
              <Button
                ...
                sx={{
                  ...
                  fontWeight: 600,
                  textTransform: 'none',
                  bgcolor: selectedTable ? '#10b981' : 'rgba(255, 255, 255, 0.8)',
                  color: selectedTable ? 'white' : 'text.primary',
                  border: selectedTable ? 'none' : '1px solid rgba(0, 0, 0, 0.12)',
                  boxShadow: selectedTable 
                    ? '0 4px 12px rgba(16, 185, 129, 0.3)' 
                    : '0 2px 8px rgba(0, 0, 0, 0.08)',
                  '&:hover': {
                    bgcolor: selectedTable ? '#059669' : 'rgba(0, 0, 0, 0.04)',
                    transform: 'translateY(-1px)',
                    boxShadow: selectedTable 
              ...
              >
                {selectedTable ? `Table ${selectedTableName}` : 'Select Table'}
              </Button>
            )}
      ...
      <Box sx={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          <Box sx={{ flex: 1, p: 2, overflow: 'auto' }}>
            ...
            {filteredProducts.length === 0 && selectedDepartment && (
              <Box 
                sx={{ 
                  display: 'flex', 
              ...
              >
                ...
                <Typography variant="body2">
                  {selectedCategory ? 'Try selecting a different category' : `No products in ${selectedDepartmentName} department`}
                </Typography>
              </Box>
            )}
          </Box>
