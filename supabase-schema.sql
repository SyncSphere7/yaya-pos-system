...
create table public.users (
    ...
    pin varchar(4), -- 4-digit PIN for staff login
    pin_enabled boolean default false, -- Enable PIN login for this user
    employee_id varchar(20), -- Optional employee ID for display
    is_active boolean default true,
    created_at timestamp with time zone default timezone('utc'::text, now()) not null,
    updated_at timestamp with time zone default timezone('utc'::text, now()) not null
);

-- Departments table (Restaurant, Bar, Fumes)
...
-- Categories table (now belongs to departments)
...
-- Tables table (for all departments)
